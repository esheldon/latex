...
lensing shear effects to probe the expansion history and growth rate of massive
structures in our universe.  This work has two aspects: development of methods
to measure shear and interpretation of the shear in terms of cosmology.  I am a
DES ``builder'' and a leader of the DES lensing effort, bringing data rights
for myself and a research associate.
...
measurements of cluster lensing in the Sloan Digital Sky Survey (SDSS), which
are most sensitive such measurements to date, and naturally leads to future
work on projects such as the Large Scale Synoptic Telescope\cite{lsstweb}.

I am finishing the prototyping stage of a promising new algorithm to measure
lensing effects from astronomical images.  This method shows potential to meet
...
processing units, making analysis of the full DES possible with a small amount
of computing resources.  In addition, in collaboration with Mike Jarvis of
UPenn, I am continuing to test and tune the de-facto algorithm and pipeline we
have developed for DES.
...
associate and I will process the data as it arrives to measure gravitational
shear.  We will perform science analysis of the shear to measure the masses of
galaxy clusters and infer the properties of Dark Energy.

%These analyses will most likely
...
The work on DES will have to primary aspects:  development of techniques to
measure gravitational shear and science analysis in terms of Dark Energy.
I will focus primarily on developing measurement techniques and processing
the data.  I will guide a research associate to analyze these measurements
to extract cosmological parameters.  In the following sections I will
describe the current state and planned activities.
...
A research associate and I will process the gravitational shear measurements to
calibrate the masses of the galaxy clusters used in the cosmological analyses.
We will also participate in the measurement of shear-shear correlations (cosmic
shear).  These are critical components of the DES mission.  
...
the PSF.  Right panel: PLACEHOLDER FIGURE\devprof\ galaxies fit by a Gaussian mixture with the
...
expected 500 million galaxies will be observed on $\sim$ 10 separate exporues,
...
I have made great gains in speed by using a graphics processing unit (GPU).
...
the GPU, returning of order 10 numbers each, minimizing the overhead moving
data back and forth.  I have found that each likelihood calculation is faster
by a factor of \speedup\ compared to the CPU code, primarily due to
parallelization of the pixel rendering to individual GPU cores.  Additionally,
my method of exploring the likelihood uses an affine invariant Markov
chain\cite{GoodmanWeare10} with twenty separate ``walkers'' that can be
evaluated independently at each step.  Thus, although the pixel parallelization
only results in a speed increase of about a factor of \speedup\ over the highly
optimized CPU code, I have seen another factor of twenty gain by parallelizing
the walkers.  More walkers could be used for better parallelization, but the
tradeoff is a more costly burnin for the Markov chain, as the required burnin
does not decrease inversely with the number walkers.  Note a ``high-end''
graphics card is required to get this speedup; I tested an Nvidia 2050. The
comparison CPUs are 2.7 GHz intel Xeon processors.
...
can be attached to each node.  If take the measured increase in speed of
...
increase in speed over the combined 12 cpu cores on those systems for less than
...
25x25 pixels, it is rather slow to upload this to the GPU memory. Also note a
high performance distributed file system is needed to get the required data
throughput, but the cosmology group at BNL uses the Hadoop Distributed File
System, which provides sufficient throughput.
...
We will fund our existing postdoc, Andres Plazas, for the first two years of
the award and a second postdoc will begin in the third year.
...
cosmology, and I expect the postdocs will primarily work on that analysis.
This involves measuring the ``cluster-mass cross-correlation function'' from
the shear, interpreting this in terms of mass, and incorporating this into an
overall measurement of cosmology as outlined in section \ref{sec:sdssold}.
...
may increase slightly and 2) it is more cost-effective and efficient to spread
the required disk to more machines.  We desire fairly large memory on the
...
and covers several tens pointings, with about 10 exposures per pointing.  Tests
...
\$3,600.  The total cost for ten machines is \$88,000 plus overhead.  These can
be purchased over the five year survey.  There is potential for increase in
speed per GPU, but this is difficult to predict currently as scientific GPUs
are a new technology.  I prefer to quote the current performance profile.
