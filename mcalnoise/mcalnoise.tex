...
\rangle^{S+}$ represents the mean of the unsheared measurements, with selection
placed on the sheared parameters. The $\langle E \rangle^{S-}$ represents the
mean of the unsheared sheared measurements, with selection placed on the
negatively sheared parameters.  In practice, we can store the sheared and
unsheared data and perform separate selections and statistics on those values
to compute the various ensemble averages.
