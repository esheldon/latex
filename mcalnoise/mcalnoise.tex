...
more complex when modeling the effect of the PSF.  We instead explore
...
the assumptions these correction methods.  In \S \ref{sec:addn} we explore a
simpler approach to correcting the noise.

\subsection{Adding a Sheared, Correlated Random Noise Field} \label{sec:addn}
...
statistically remove the effects of sheared, correlated noise.  The
result, $\tilde{I}(\gamma)$, is an approximation for the image
without correlated noise:
\begin{equation}
    \tilde{I}(\gamma) = I_o(\gamma) + \tilde{\eta}(-\gamma),
\end{equation}
We used these $\tilde{I}(\gamma)$
to measure the shapes and responses used in shear recovery.
...
grossly incorrect models for all of our simulations.  Naively using these
incorrect models results in a large model bias, and large uncorrected PSF
anisotropy.  We furthermore expect large {\em ordinary} noise bias at the low
S/N levels used in the simulations.
...
parameters.  For the flux and size we deliberately used priors that were wider
than truth by 20\%.  For the ellipticity we used $\sigma=0.3$ instead of the
true $\sigma=0.2$.
...
\begin{table*}
    ...
    \begin{tabular}{ |l|  r|r|r| |c|  r|r|r|}
        ...
        Sim         & $m$             & $c_1 \times 10^4$ & $c_2 \times 10^4$ & method   & $m \times 10^{3}$ & $c_1 \times 10^4$ & $c_2 \times 10^4$ \\
        \hline
        BD          & $0.10 \pm 0.00$ & $-0.3 \pm 0.2$    & $0.2 \pm 0.2$     & detrend  & $ 0.5 \pm 0.4$    & $-0.3 \pm 0.2$    & $0.2 \pm 0.2$  \\
        BDStar      & $0.12 \pm 0.00$ & $-0.3 \pm 0.3$    & $0.3 \pm 0.3$     & detrend  & $ 0.5 \pm 0.5$    & $-0.2 \pm 0.2$    & $0.2 \pm 0.2$  \\
        ...
        RG          & $0.12 \pm 0.00$ & $-0.3 \pm 0.3$    & $0.0 \pm 0.3$     & noise    & $0.5 \pm 0.6$     & $-0.4 \pm 0.3$    & $0.6 \pm 0.3$ \\
    \end{tabular}
\end{table*}
