...
mysteries: dark matter and dark energy.  Both phenomena have been understood by
studying large scale dynamical systems:  dark matter was discovered by
examining the internal dynamics of the Coma galaxy cluster (XXX Zwicky), and
later confirmed with precision measurements such as the dynamics within
galaxies (XXX Rubin).  In essence, assuming standard gravity holds, there is
much more mass than we can see, and we simply call this missing mass dark
matter.  Dark energy was discovered by studying the dynamics of the universe
itself (XXX ref).  In similar fashion, assuming standard gravity holds, the
universe is apparently accelerating at late times, rather than decelerating as
we would expect given the known mass in the universe, including dark matter,
and we call the cause of this acceleration dark energy (XXX ref).

Weak gravitational lensing is a phenomena that can be used to study both dark
matter and dark energy (XXX ref).  Light passing massive objects undergoes an
apparent deflection, and the amount of this deflection depends on all the mass
in the ``lens'', both luminous and dark. The effect is independent of the
dynamical state of this mass.   This deflection distorts the apparent shape of
extended objects such as galaxies, an effect called ``shearing'', and these
distortions are spatially correlated.  Studying the correlations of these shape
distortions, one can infer the distribution of matter anywhere in the universe
in principle (XXX ref).  Because the effect depends on the geometry of the
lens-source system, the expansion history can also be inferred if the redshifts
of the lensing material and sources are known, and thus weak lensing is also
sensitive to dark energy (XXX ref).  Lensing can also be used to study dark
energy through the grown of massive structures (XXX ref).

Pioneered as a measurement technique in the late 1980s and 1990s, weak lensing
was quickly recognized to be complimentary to more standard dynamical
techniques (XXX ref, e.g. Tyson 84).  In particular, because the effect is
independent of the dynamical state of the lensing mass, it can be used to study
the distribution of mass on very large scales, much larger than the typical
regions over which systems can be said to be in dynamical equilibrium (XXX ref
fisher).
...
shapes of background galaxies at the percent level (XXX cite).    As an
intermediate goal in studying dark energy over the next decade using weak
lensing, we would like to measure this percent level signal with an accuracy of
a part in a thousand (XXX ref).
...
brightness profile, convolved by an estimate of the point-spread function.  The
shear estimator, the estimated ``shape'' of the galaxy, is then determined from
the parameters of this model.  This method is limited for two primary reasons:
First, in order for the shear inference to be accurate, the model must very
accurately reproduce the galaxy profile before convolution by the PSF (XXX
ref), which requires a large number of parameters.  If the model cannot
sufficiently reproduce the galaxy the estimator is said to exhibit ``model
bias''.  The second is that the effect of noise on the non-linear fitting is
significant for low signal-to-noise (\snr) images, and this effect is difficult
to predict (XXX ref).  This ``noise bias'' is exacerbated by the large number
of parameters required to represent the galaxy.
...
calibration based image simulations (XXX ref).  These simulations must include
...
function (XXX ref).  The simplest estimator would use unweighted moments.
However, in the presence of noise, the measurement error in these moments
diverges (XXX ref).  To address this issue a weight function can be applied
to suppress parts of the image dominated by noise.

There have been a few approaches to using weighted moments.  One can use a
round weight, which has the desirable property of not correlating with the
shape of the PSF (XXX ksb).  Another involves adapting the weight function to
match the object, which has the advantage of increasing the sensitivity of the
estimator to the shear compared to a fixed weight (XXX ref). Such methods can
be made quite accurate for high \snr\ images, with little model bias (XXX ref
yuki).  Another method involves working in Fourier space, and shearing the
coordinate system until the weighted moments vanish, rather than adapting the
moment; this method can also have little model bias because it is the shear of
the coordinate system that is iterated rather than a model for the galaxy.
However, each of these methods still suffer the ``noise bias'' found in model
fitting methods, due to the non-linear fitting involved with the measurement.
Without further development, these methods are not accurate at the part in a
thousand level (XXX ref).

Some methods have recently been developed to avoid these biases.  The first we
will discuss involves measuring the moments in Fourier space, after subtracting
off an appropriate noise power spectrum.  A circular weight is used to avoid
non-linear fitting issues.  These un-normalized moments are then summed
directly to provide a shear estimator (XXX ref).  This method has proven very
accurate, but because the moments are not normalized, the weighting is
essentially by flux, weighting to the brightest galaxies, which is highly
sub-optimal. Attempts to use different weighting have not yet been demonstrated
to be accurate (XXX ref).

The second is a promising new method for shear inference introduced by (XXX
ref).  A rigorous Bayesian framework was developed in which prior information
on galaxy images from a high \snr\ deep data set is combined with knowledge of
how a particular measurement (e.g. a model fit or moments) responds to a shear
in the absence of noise.  The method has been shown to be accurate in a highly
controlled, but unrealistic numerical experiment using model fitting techniques
(XXX ref sheldon).  A more practical version using un-normalized moments in
Fourier space was tested in (XXX ref).  While the accuracy in that test fell
somewhat short of the part-in-a-thousand target, the method shows great
promise, since the required prior information from deep data should in
principle be available to current and future surveys (XXX ref).

Thirdly, \Mcal\ \citep{HuffMcal} is a new shear measurement technique designed
to address the shortcomings of standard shear measurement techniques, without
requiring prior information.  The essence of \mcal\ is to directly measure
how a particular measurement responds to a shear, eliminating both model bias
and noise bias.  \Mcal\ can in principle be applied to calibrate any shear
estimator, including shapes derived from model fitting or weighted moments.

A similar idea was introduced by \cite{Kaiser2000}, a work with great
foresight.  Much of the treatment therein was quite general,  with most of the
detailed calculations focusing on weighted second moment estimators.  The
approach introduced by \citep{HuffMcal} is intentionally agnostic about the
underlying shear estimator, with a focus on generic image manipulations
efficiently performed on current computers.
...
dominated by small faint galaxies, many of which cannot be reliably detected or
measured.  In this work we use a much more challenging set of simulations.
...
PSF is well determined.  There may also be missing data in the images, such as
bad pixels or columns; we show that for typical observing conditions this
...
XXX Say where things are in the paper.
...
by the PSF.  A slightly larger PSF is used in order to suppress Fourier modes 
...
effects.  We introduce the following notation regarding the shear estimator and
the selection function:  We write the ellipticity measured from a sheared image
as $\mest^+$ and $\mest^-$, for measurements on positive and negatively sheared
images respectively.  Similarly, we can select objects based on sheared
parameters, which we denote $S^+$ and $S^-$, the probability of selection after
a positive or negative shear is applied, respectively.
...
which we can, without loss of generality, refer to as the ellipticity.  As a
shorthand we will write this selection function as $S(\mest)$, the probability
of selecting an object with ellipticity \est, although the selection may be
indirect, for example a cut on \snr.  This selection function could also
represent some kind of weighting scheme that indirectly weights by ellipticity.
...
Note selections that produce additive effects are not addressed in the above.
...
such biases.  We will test the importance of this effect using image
simulations.
...
$\left< R_\beta \right>$ are statistically identical, and are each equal to the
...
\snr\ galaxies, where shape noise dominates, this results in a 40\% decrease in
effective \snr. However, for faint galaxies, which dominate the sample, the
shape measurement noise is comparable to the shape noise, so the decrease in
effective \snr\ is a modest 20\%.  Nevertheless, it is worthwhile to explore
corrections that do not involve adding significant noise.
...
response is representative of the correct noise level.  To reduce the noise in
the fits, multiple randomized noise fields could be generated and fit, and the
results averaged.  This would improve the precision, at the cost of increased
computation time.
...
a range of parameters.  In order to show detailed internal structure, for
visual purposes, we created models larger than any in the actual simulations we
ran for testing, in which were mostly smaller than the PSF.
...
threshold in order to minimize the bias due to removal of these
galaxies.  See \S \ref{sec:shear_recover} for details.
...
which suggests our procedure of reconvolving by a symmetrized PSFs worked
as expected.
