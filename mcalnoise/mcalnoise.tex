...
confirmed with higher precision by studying the dynamics in galaxies (XXX
Rubin).  In essence, assuming standard gravity holds, there is much more mass
than we can see, and we simply call this missing mass dark matter.  Dark energy
was discovered by studying the dynamics of the universe itself (XXX ref).  In
similar fashion, assuming standard gravity holds, the universe is apparently
accelerating at late times, rather than decelerating as we would expect given
the known mass in the universe, including dark matter, and we call the
cause of this acceleration dark energy (XXX ref).
...
in the ``lens'', both luminous and dark, and the effect is independent of the
...
sensitive to dark energy (XXX ref).

Pioneered as a measurement technique in the late 1990s, weak lensing was
quickly recognized to be complimentary to more standard dynamical techniques
(XXX ref).  Thus far, progress in measurement has been limited by technical
challenges, which we will outline below.  To set a scale of reference, the weak
...
shear estimator, the estimated ``shape'' of the galaxy, is then based on the
parameters of this model.  This method is limited for two primary reasons:  The
first is that the model must very accurately reproduce the underlying galaxy
profile and shape in order for the shear inference to be accurate (XXX ref),
which requires a very large number of parameters.  If the model cannot
...
calibration.  As such, the science of shear from model fitting is essentially
that of accurately simulating observations of the universe.
...
coordinate system rather than adapting the moment; this method does not suffer
model bias because it is the shear of the coordinate system that is iterated
rather than a model for the galaxy.  However, each of these methods still
suffer the ``noise bias'' found in model fitting methods, due to the non-linear
fitting involved with the measurement. Without further development, these
methods are not accurate at the part in a thousand level (XXX ref).
...
requiring prior information.  The essence of \mcal\ is to directly measure how
a particular measurement responds to a shear, eliminating both model bias and
...
measured. Additionally, during estimation of a shear statistics, further
selection is often required, such as cutting or binning based on galaxy
properties, which can introduce significant selection biases.  And the \mcal\
producure itself produces correlated noise in the image, which becomes a
dominant source of error for low \snr\ galaxy images.

In this study we introduce a simple correction for the correlated noise effect
in \mcal\ measurements.  Additionally, we introduce a formalism to deal with
selection effects, for example cuts on \snr\ and galaxy size, that are required
for any practical analysis.  We additionally examine the effects of stellar
contamination and missing data.  We show that, in all scenarios we tested,
\mcal\ is indeed accurate at the part in a thousand level.

XXX Say where things are.
