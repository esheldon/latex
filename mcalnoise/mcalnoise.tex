...
Consider a selection that modifies the distribution of the estimator $E$ (
which could be some measure of ellipticity). For shorthand we will write this
selection function as $S(E)$, although the selection may be indirect, for
example a cut on \snr.  Ignoring additive biases, we can rewrite equation
\ref{eq:basicestimator} as
\begin{align}
    \langle E \rangle &\approx \gamma \int S(E) \frac{\partial  P(E) E  }{\partial \gamma} dE \nonumber \\
                      &= \gamma \int S(E) P(E) \frac{\partial E }{\partial \gamma} dE + \gamma \int S(E) E \frac{\partial P(E) }{\partial \gamma} dE \nonumber \\
                      &\approx \gamma \left[ \langle R \rangle + \int S(E) E \left( \frac{P(+\gamma) - P(-\gamma)}{\Delta \gamma}\right) dE \right] \nonumber \\
...
\rangle^+$ represents the mean of the estimator based on the sheared probability
distribution, and $\langle E \rangle^-$ represents the mean over the negatively
sheared distribution.  The first term
$\langle R \rangle$ is the normal \mcal\ response, averaged over the unsheared distribution.  In practice,
...
sources, which means the true shear is will be different even with out any
selection bias.  This must be taken into account separately. We will not
treat this effect in what follows.
