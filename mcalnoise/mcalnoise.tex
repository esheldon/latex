...
We can extend this formalism to 2-point functions.  Consider a function
in which we correlate two components of a shear estimator $\mest_\alpha$
and $\mest_\beta$.  We can write the estimator, including 
selection, as
\begin{align}
    \xi &= \langle S_{\alpha\beta} \mest_\alpha \mest_\beta \rangle \nonumber \\
        &= \int P(\mest_\alpha \cdot \mest_\beta) S(\mest_\alpha, \mest_\beta) \mest_\alpha \mest_\beta  d\mest_\alpha d\mest_\beta  
\end{align}
In what follows, we will shorten the notation to simply $P$ and $S$,
with the understanding that these apply to both $\alpha$ and $\beta$.
Differentiating with respect to shear in one of the components, we find
\begin{align}
    \frac{ \partial \xi}{\partial \gamma}\bigg|_{\gamma=0} &= \int \frac{\partial P S \mest_\alpha \mest_\beta}{\partial \gamma}\bigg|_{\gamma=0} d\mest_\alpha d\mest_\beta \nonumber \\
    &= \int \left[ S \frac{\partial P \mest_\alpha \mest_\beta}{\partial \gamma}\bigg|_{\gamma=0} + P \mest_\alpha \mest_\beta \frac{\partial S}{\partial \gamma}\bigg|_{\gamma=0}  \right] d\mest_\alpha d\mest_\beta \nonumber \\
\end{align}
Approximating the derivatives using finite difference, and  using the
notation for sheared quantities and averages introduced above, we can rewrite this as
\begin{align}
    \frac{\partial \xi}{\partial \gamma}\bigg|_{\gamma=0}
    &\approx
    \frac{\langle \mest_\alpha^+ \mest_\beta^+ S \rangle - \langle \mest_\alpha^- \mest_\beta^- S \rangle}{\Delta \gamma} + 
\frac{\langle \mest_\alpha \mest_\beta S^+ \rangle - \langle \mest_\alpha \mest_\beta S^- \rangle}{\Delta \gamma} \nonumber \\
    &\equiv R^{2pt} + R^{2pt}_S.
\end{align}
The first term is the response of the 2-point function to a shear $R^{2pt}$,
and the second term is the response of the 2-point function to selections $R^{2pt}_S$.
