...
by $\sqrt{2}$, but this not a severe limitation.  As we will see below, \mcal\
can be used to recover the shear accurately at very low \snr.  As an example,
if one plans to make cuts at $\snr\ > 10$ for a particular analysis, one can
simply reduce the cut by $\sqrt 2$ to $\snr\ > 7$ without expected loss of
accuracy.  Also, for faint galaxies, which dominate the sample, the shape
measurement noise is comparable to the intrinsic shape noise, so one might
expect the increase in effective noise to be a more modest 20\%.  But in fact
most of the faint sources have quite small shear response, so their effective
weight in the shear recovery is already small.

We performed a test where the above correction was made with $\snr\ > 7$ and
compared to a ``perfect'' \mcal\ without correlated noise (all noise was added
after the image manipulations) and cut at $\snr\ > 10$. For the simulations we
describe below, we found the uncertainty in the shear recovery was only increased
by about 7\%.

Furthermore, for correlation function measurements over large scales, such as
shear-shear measurements, sample-variance will dominate and this small increase
in noise will be unimportant.  Nevertheless, it is worthwhile to explore
...
with correlated noise effects.  This extra noise only increases the uncertainty
in the shear recovery by a modest amount in our tests, about 7\%, and should be
even less important for sample-variance dominated studies such as shear-shear
correlations on large scales. Nevertheless we have identified the development
of a more precise correlated noise correction as a priority going forward.
