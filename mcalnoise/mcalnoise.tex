...
the above correction.  This was compared to a ``perfect'' \mcal\ procedure
without correlated noise. To accomplish this, we only added noise after the
\mcal\ image manipulations were performed.  In both cases we placed a cut at
$\snr\ > 10$. For the simulations described in \S \ref{sec:bdksim}, and the
fitting used in \S \ref{sec:modelfit}, we found that the uncertainty in the
shear recovery was increased by $\sim$\degrade.
