...
objects or regions of sky. Note the probability distribution $P(e_\alpha e_\beta)$ is joint in
the two ellipticities. Including selection effects, this becomes
\begin{align}
...
\begin{align}
    \xi \approx \left< \frac{\partial^2 \xi}{\partial \gamma_\alpha \partial \gamma_\beta} \gamma_\alpha \gamma_\beta \bigg|_{\gamma=0} \right>
\end{align}
...
selections in each object.  The last term represents the response of the
estimator to shearing.  The cross terms represent the response to shearing
in one object and selection in another.
...
\begin{align}
    \int d e_\alpha & d e_\beta  \frac{\partial S_\alpha}{\partial \gamma_\alpha} \frac{\partial S_\beta}{\partial \gamma_\beta}  P_{\alpha \beta} e_\alpha e_\beta \bigg|_{\gamma=0} \approx \nonumber \\
...
respectively.  These are three separate measurements to be made:  the first and
...
finite differences:
\begin{align}
    \int d e_\alpha  d e_\beta  2 S_\alpha \frac{\partial S_\beta}{\partial \gamma_\beta} & \frac{\partial P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha} \bigg|_{\gamma=0} \approx \nonumber  \\
...
\begin{align}
    \int d e_\alpha  d e_\beta S_\alpha S_\beta & \frac{\partial^2 P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha \gamma_\beta} \bigg|_{\gamma=0} \approx \nonumber \\
...
In figure \ref{fig:parametricgals} we show some example simulated galaxies with a range
of parameters.  These galaxies are larger than any galaxies in the actual
simualtions we ran, otherwise the interesting structure would be smeared
out by the PSF.
...
\begin{figure*}
    ...
    \caption{Example simulated galaxy images.}
