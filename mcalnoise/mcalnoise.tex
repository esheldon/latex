...
Suppose we have a biased shear observable $E$.  This may be some estimate
of ellipticity.  We can expand this observable
in a Taylor series around the true shear
\begin{align} \label{eq:Eexpand}
    E(\gamma) &= E|_{\gamma=0} + \gamma ~ \frac{ \partial E }{ \partial \gamma }\bigg|_{\gamma=0}  + ... \\
\end{align}
We can use the ensemble mean of such measurements over a large number of objects
to estimate the mean shear:
\begin{align}
    \langle E \rangle &= \langle E \rangle |_{\gamma=0} + \langle \gamma \frac{ \partial E }{ \partial \gamma } \rangle \bigg|_{\gamma=0} + ...
\end{align}
For constant shear, and assuming the estimator gives zero for $\gamma=0$, we can rewrite this as
\begin{align}
\langle E \rangle &\approx  \gamma ~ \langle \frac{ \partial E }{ \partial \gamma } \rangle \bigg|_{\gamma=0}  \\
...
Another response can be derived for errors in the correction for PSF
anisotropy, resulting in a non-zero value for $\langle E \rangle|_{\gamma=0}$.  This
...
\begin{equation}
    \gamma = \frac{\langle E \rangle - \langle \mbox{\mcalRpsf} \rangle \langle e^{p} \rangle}{ \langle \mbox{\mcalR} \rangle}.
\end{equation}
Finally, the estimator $E$ has two-components, and the response \mcalR\ is
a $2 \times 2$ matrix. Thus the full solution is given by the matrix
equation
\begin{align}
    \gamma &= 
    \langle \mbox{\mcalR} \rangle^{-1} \left[ \langle E \rangle - \langle \mbox{\mcalRpsf} \rangle \langle e^{p} \rangle \right].
\end{align}
...
measurement $E$, although the selection may be indirect, for example a cut on
\snr.  The mean ellipticity over an ensemble can then be written as 
\begin{align}
    \langle E \rangle = \int S(E) P(E) E dE,
\end{align}
where $P(E) = d N / d E$ is the normalized probability
distribution for the measurement.
To leading order, we can write the multiplicative response of the mean to
an applied shear as
\begin{align}
...
\end{align}
%By symmetry, we expect the mean derivative of the distribution 
%$\partial P / \partial \gamma|_{\gamma=0}$ to vanish.  However,
%after a selection this symmetry may be broken.
...
\end{align}
where $S(\gamma)$ is the probability of selection after a small shear is
applied to the image.  We can thus interpret the integral over $S(+\gamma)$
and $S(-\gamma)$ as the mean
of the measurement under positive and negative shear,
\begin{align}
\frac{\partial \langle E \rangle}{\partial \gamma}\bigg|_{\gamma=0} &\approx
...
represents the mean of the sheared measurements, with selection placed on the
sheared parameters. The $\langle E \rangle^-$ represents the mean of the
negatively sheared measurements, with selection placed on the negatively
sheared parameters.  The first term $\langle R \rangle$ is the normal \mcal\
response, with selection placed on the unsheared parameters.  In practice, we
can store the sheared parameters used to measure \mcalR, and perform separate
selections and statistics on those values to compute \mcalRS.
...
threshold, some objects would be missed whose measurements would  have
met the selection criteria after shearing.
