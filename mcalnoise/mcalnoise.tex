...
In what follows we test the formulas for mean shear.  We will explore the
...
As discussed in \S \ref{sec:mcal}, if a shear estimator has
...
smaller objects are higher \snr\ than larger objects with the same flux,
...
We apply a pre-selection at \snr$ > 5$.  In order to avoid this bias,
we will apply further cuts on the \snr\ above this threshold.
...
of the PSF. We created this PSF by added the PSF image to itself, rotated by
...
Gaussian, divided by half the trace $T$, which is the mean extent of the
...
case.  Without applying the \mcal\ responses, the multiplicative bias $m$ is of
...
Inn table \ref{tab:results_sel}, we show the results for different \snr\
...
We find significant multiplicative selection effects cutting on \snr\
thresholds.  These biases are well above the requirements for current and
future experiments, and thus must be corrected.  We find \mcal\ corrects
these biases to within the requirements of future surveys in all cases.
...
stars, but the noise did increase by \starnoiseincrease.
...
\ref{sec:selection}, but only if the selection is based on quantities
measured on sheared images.  If the selection must be performed
outside of the \mcal\ process, bit may be better to avoid it altogether.
