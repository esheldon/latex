...
\newcommand{\Aslope}{$-0.097 \pm 0.002$}
\newcommand{\Aoffset}{$-0.0018 \pm 0.0004$}
...
in the image $n^2$ \citep{HirataCorrNoise}.
\begin{align} \label{eq:scaling}
    \mbox{\mcalRo} &= R + \mbox{\mcalRnoise}  \nonumber \\
                   &= R + A n^2
\end{align}
We can add a small amount of noise $\Delta n$ to the image, run the new image
through the \mcal\ process, and measure $R^{\mathrm{before}}$.  We can also
add this noise {\em after} the original image  has been run through \mcal, and
measure $R^{\mathrm{after}}$.  The difference between is
then 
\begin{align}
    \Delta R &\equiv R^{\mathrm{before}} - R^{\mathrm{after}}  \nonumber \\
             &\simeq A \Delta n^2
\end{align}
where $\Delta n^2 \equiv (n + \Delta n)^2 - n^2$.
Given $A$, we can estimate the contamination by correlated noise
\begin{align}
    \mbox{\mcalRnoise} \simeq A n^2
\end{align}
In practice, the \mcal\ processing steps move the noise around, so we do not
necessarily expect the \mcalR\ appearing in equation \ref{eq:scaling} to
equal the exact response of the estimator in the absence of
correlated noise.  We may thus find a difference between the inferred
\mcalRnoise\ and the true value.  If this offset is sufficiently constant over
the range of noise used for detrending, we can determine it from a linear fit
between $\Delta R$ and
$\Delta n^2$:
\begin{align}
    \Delta R &= A \Delta n^2 \nonumber + \Delta R(n=0) \\
    \Rightarrow \mbox{\mcalRnoise} &\simeq A n^2 - \Delta R(n=0).
\end{align}
It is not yet clear if subtracting this offset is needed.
...
\subsection{Detrending Corrections}

We apply the de-trending technique to image simulations.  These simulations
make use of parametric models.  The galaxies are bulge+disk models with both
components having the same ellipticity and half light radius, drawn from a
log-normal distribution centered at 0.7 arcsec, with scatter of 0.13 arcsec.
The bulges are offset uniformly up to a scale length.  The ellipticities
were drawn from the simple model presented in \cite{ba14}, with $\sigma=0.2$.
The PSF is an elliptical
Gaussian with FWHM 0.9 arcsec.  The pixel scale was set to 0.265 arcsec.  The S/N
distribution matches real DES data, with $S/N \gtrapprox 10$.  Galaxies
were sheared with 300 different shears, ranging from 0.01 to 0.08 in amplitude.
...
The results showing $\Delta R$ vs $\Delta n^2$ are shown in figure
\ref{fig:detrend}.  The trend is linear in $\Delta n^2$ as expected, with a
slope $A = $\Aslope.  A significant offset is measured, $\Delta R(n=0) =
$\Aoffset.  It is not clear, however, if this is just an offset to the
line or if there is actually some curvature in this plot.  Another run
using smaller noise differences may clarify this point.

\begin{figure}
    \includegraphics[scale=0.45]{run-bd13mcal-dt01-Rnoise-detrend.eps}

    \caption{Trend of $\Delta R$ with noise squared, relative to the original
    noise level in the image.  The trend is linear in $\Delta n ^2$ as
predicted, with slope \Aslope. A significant offset at
$\Delta (n=0)$ is detected.}

\label{fig:detrend}
\end{figure}

Applying these corrections to $R$, we recover the shear within the
noise.  Without subtracting the offset, we find
\begin{align}
    m &=  (-1.0 \pm 2.0) \times 10^{-3} \nonumber \\
    c &=  (-3.4 \pm 7.0) \times 10^{-5}.
\end{align}
Subtracting the offset we find
\begin{align}
    m &=  (1.5 \pm 2.0) \times 10^{-3} \nonumber \\
    c &=  (-3.4 \pm 7.0) \times 10^{-5}.
\end{align}
Larger and more complex simulations are needed to assess if subtraction of the
offset is needed.  A larger simulation, using COSMOS galaxies and realistic DES
PSF is running.
