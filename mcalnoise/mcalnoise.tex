...
If we assume the mean shear is zero over a large area of sky, the leading
surviving term in the Taylor expansion is a second derivative
...
\end{align}
As with the mean shear estimator, equation \ref{eq:basictwopt} looks like the
...
shear, \probe\ is separable such that $\mbox{\probe}|_{\gamma=0} = P(e_\alpha) P(e_\beta)$.
Given this assumption, the following identities also hold
\begin{align}
    \frac{\partial \mbox{\probe}}{\partial \gamma_\alpha}\bigg|_{\gamma=0} &= P(e_\beta) \frac{\partial P(e_\alpha) }{\partial e_\alpha} \frac{\partial e_\alpha}{\partial \gamma_\alpha} \nonumber \\
    \frac{\partial^2 \mbox{\probe}}{\partial \gamma_\alpha \partial \gamma_\beta}\bigg|_{\gamma=0} &= \frac{\partial P(e_\alpha) }{\partial e_\alpha} \frac{\partial P(e_\beta) }{\partial e_\beta}  \frac{\partial e_\alpha}{\partial \gamma_\alpha} \frac{\partial e_\beta}{\partial \gamma_\beta}
\end{align}
As a consequence, the response itself can be completely factored such that
\begin{align}
    \int d e_\alpha  de_\beta  & \frac{\partial^2 S_\alpha S_\beta \mbox{\probe} e_\alpha e_\beta}{\partial \gamma_\alpha \partial \gamma_\beta}\bigg|_{\gamma=0} 
      \nonumber \\
