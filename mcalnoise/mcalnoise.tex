...
In practice the off-diagonal elements of the response matrix \mcalR\ are
negligible, so the correction in equation \ref{eq:rcorr} can be done
element-wise.  In fact, the diagonal elements of \mcalR\ are usually
equal, reducing the correction to division by a scalar \mcalRscalarmean.
...
image.  We can represent the series of operations on the observed image $I$.
\begin{equation}
...
As noted in \S \ref{sec:mcal}, the off-diagonal elements of the shear response
matrix \mcalRg\ are usually negligible, and the diagonal elements equal.  If
the selections are also not direction dependent, 
we can form a simpler estimator for the 2-point function,
dividing by square of the scalar mean response
%\begin{align}
