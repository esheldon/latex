...
sheared correlated noise field.  For shear estimators based on non-linear
fitting, we propose the following process:
...
\begin{equation}
    \langle R_\eta \rangle = \langle R^{model}_\eta - R^{model} \rangle.
\end{equation}
The mean measured response for an ensemble of galaxies, $\langle R \rangle$ can
then be corrected by subtracting the mean correction.
