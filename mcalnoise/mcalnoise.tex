...
will discuss \citep{Zhang2015,Zhang2017} involves measuring the moments in
Fourier space, after subtracting off an appropriate noise power spectrum.  A
circular weight is used to avoid non-linear fitting issues.  These
un-normalized moments are then either summed directly to provide a shear
estimator, or the PDF of measurements is symmetrized in order to infer the
shear, with the latter providing a more precision estimate.  This method is
quite promising, with an accuracy in idealized simulations at the part in a
thousand level.  However, as of yet no method is provided to deal with
selection effects.
...
method, without use of external simulations.  The method has been shown to be
accurate in a highly controlled, but unrealistic numerical experiment using
model fitting techniques \citep{Sheldon2014}.  A more practical version using
un-normalized moments in Fourier space was tested in \citep{bfd2016}.  While
the accuracy in that test fell somewhat short of the part-in-a-thousand
target, the method shows great promise, since the required prior information
