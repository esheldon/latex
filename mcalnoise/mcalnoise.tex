...
\begin{equation}
    R^{model}_\eta = R^{model} + \mbox{\mcalRnoise}
\end{equation}
An estimate of the contribution from correlated noise can
be calculated by averaging
\begin{equation}
    \langle \mbox{\mcalRnoise} \rangle = \langle R^{model}_\eta \rangle - \langle R^{model} \rangle.
\end{equation}
The construct this average, we repeat the model calculations using the
noise properties of every galaxy in a sample.
...
for galaxies.  I would not expect \mcalRnoise\ to cancel for 90 degree rotated
