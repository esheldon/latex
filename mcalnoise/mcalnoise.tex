...
\mcalR\ for a measurement $E$ directly from image data.  This is
...
If the PSF correction is not perfect, the leading term $\langle E \rangle|_{\gamma=0}$
...
\end{equation}
Finally, the measurement $E$ has two-components, and the response \mcalR\ is
...
\end{align}
where $P(E)$ is the normalized probability
...
selections applied to the unsheared parameters.  The second term is the
response of the measurement to selections.  We can approximate the derivative
using a finite difference:
\begin{align}
    \frac{\partial \langle E \rangle}{\partial \gamma}\bigg|_{\gamma=0}
    &\approx
    \langle \mbox{\mcalR} \rangle + \int E~P(E) \left[ \frac{ S^+ - S^- }{\Delta \gamma}\right] dE,
\end{align}
...
shear is applied.  We can thus rewrite this in terms of the mean measurement
when selections are applied to sheared parameters:
\begin{align}
\frac{\partial \langle E \rangle}{\partial \gamma}\bigg|_{\gamma=0} &\approx
    \langle R \rangle + \frac{\langle E \rangle^{S+} - \langle E \rangle^{S-}}{\Delta \gamma} \nonumber \\
       &\equiv \langle R \rangle + \langle \mbox{\mcalRS} \rangle,
\end{align}
where we have defined the selection response \mcalRS. The $\langle E \rangle^{S+}$
represents the mean of the unsheared measurements, with selection placed on the
sheared parameters. The $\langle E \rangle^{S-}$ represents the mean of the
unsheared sheared measurements, with selection placed on the negatively
sheared parameters.  In practice, we
can store the sheared and unsheared parameters and perform separate
selections and statistics on those values to compute the various 
ensemble averages.
...
threshold, some object measurements that should become detectable after
