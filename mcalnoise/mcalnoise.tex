...
There have traditionally been two approaches to measuring the effect:
measuring moments of the light distribution and fitting models.
...
derive a calibration, so model-fitters have typically resorted to calibration
based image simulations (XXX ref).  These simulations must include all the
relevant details of the real universe in order to provide an accurate
...
rather than a model for the galaxy.  However, each of these methods suffer the
...
not accurate at the part in a thousand level (XXX ref).
...
has proven very accurate, but because the moments are not normalized, the
weighting is essentially by flux, weighting to the brightest galaxies, which is
highly sub-optimal. Attempts to use different weighting have not yet been
demonstrated to be accurate (XXX ref).

A promising new method for shear inference was introduced by (XXX ref).  A
rigorous Bayesian framework was developed in which prior information on galaxy
images from a high \snr\ deep data set is combined with knowledge of how a
particular measurement (e.g. a model fit or moments) responds to the shear in
the absence of noise.  The method has been shown to be accurate enough for
future surveys in a highly controlled, but unrealistic numerical experiment
using model fitting techniques (XXX ref sheldon).  A more practical version
using un-normalized moments in Fourier spaces, similar to (XXX ref) was tested
in (XXX ref).  While the accuracy in that test fell somewhat short of the
...
address the shortcomings of standard shear measurement techniques, without
...
dominated by small faint galaxies, many of which are too faint to be reliably
detected or measured, requiring some kind of selection that can introduce
significant biases.  Additionally, for low \snr\ images the effects of
correlated noise that arise in the \mcal\ procedure, described below, become
dominant.
...
missing data.  We show that, in all scenarios we tested, \mcal\ is indeed
accurate at the part in a thousand level.
