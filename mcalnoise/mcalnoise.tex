...
components had the same ellipticity and half light radius \hlr. The \hlr\ was
drawn from a log-normal distribution centered at 2.0 pixels, with scatter of
0.5 pixels.  The bulges were offset in a random direction from the disk, with
offsets drawn from a uniform distribution between zero and the \hlr.  The
...
We included ``stars'' in the simulations.  These stars were simply drawn as
PSFs, with the same flux distribution as used for the galaxies.

We also masked parts of the data in order to mimic certain effects in real
images. This is an important effect to simulate, because the Fourier transforms
