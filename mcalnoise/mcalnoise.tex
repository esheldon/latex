...
effect called ``shear'', and the distortions of a set of objects
is generally spatially correlated.
Studying the correlations of these shape distortions, one can infer the
correlations in the matter. One can study these correlations at any place
in the universe, including the locations of galaxies and cluster of
galaxies (XXX ref), voids (XXX ref), and even without reference
to known objects, the so-called ``cosmic shear'' (XXX ref).
...
requiring significant prior information about galaxy properties.  The essence of
\mcal\ is to directly measure how a particular measurement responds to a shear,
addressing both model bias and noise bias.  \Mcal\ can in principle be applied
to calibrate any shear estimator, including shapes derived from model fitting
or weighted moments.
...
generic image manipulations efficiently performed on current computers.  Some
...
\citep{HuffMcal}.  However, the simulations used in that study \citep[based on those
used in][]{great3} used fairly high \snr\ galaxy images, and the galaxies were
...
resolution of the original image.  This process is repeated for a
...
\subsection{Modifications to the Shear Inference}
...
negatively sheared images respectively.  We denote the probability that an
object is selected as $S$;  we can also select objects based on sheared
...
following:
\begin{enumerate}
	\item The mean ellipticity, including selections $\langle e \rangle^S$
	\item The mean ellipticity, including selections, measured on sheared images $\langle e^+ \rangle^S$, $\langle e^- \rangle^S$. Alternatively, form 
        responses for each galaxy \mcalRg\ and average those
    \item The mean ellipticity after selections based on measurements
