...
effect called ``shear'', and these distortions are spatially correlated.
Studying the correlations of these shape distortions, one can infer the
distribution of matter anywhere in the universe in principle (XXX ref).
Because the effect depends on the geometry of the lens-source system, the
expansion history can also be inferred if the redshifts of the lensing material
and sources are known, and thus weak lensing is also sensitive to dark energy
(XXX ref).  Lensing can also be used to study dark energy through the grown of
massive structures (XXX ref).

Pioneered as a measurement technique in the 1980s and 1990s, weak lensing
...
images respectively.  We denote the probability that an object is selected as
$S$;  we can also select objects based on sheared parameters, which we denote
...
We can calculate this response using quantities measured on
artifically sheared images, as presented in \S \ref{sec:mcal}.
...
\begin{align}
    ...
    \approx \frac{\langle \mest^+ \rangle - \langle \mest^- \rangle}{\Delta \gamma}
...
by averaging the individual responses.
...
\est, which we can, without loss of generality, refer to as the ellipticity.
...
The probability that an object is selected changes if it is sheared.  We can
...
We can again calculate this full response using quantities measured on
artifically sheared images. It is useful
to examine separately the response of the estimator $\mest$ to a shear, and
the response of selections to a shear:
\begin{align}
...
Thus in order to produce the desired weighted mean shear measurement, one measures 
\begin{itemize}
	\item The average ellipticity, including selections $\langle e \rangle^S$
	\item The mean of the ellipticity measured on sheared images $\langle e^+ \rangle^S$, $\langle e^- \rangle^S$. Alternatively, form 
        responses for each galaxy \mcalRg\ and average those.
    \item The mean ellipticity after selections based on positively and negatively sheared images $\langle e \rangle^{S+}$, $\langle e \rangle^{S-}$.
\end{itemize}
...
\snr\ galaxies, where intrinsic shape noise dominates, this results in a 40\% decrease in
effective \snr. However, for faint galaxies, which dominate the sample, the
shape measurement noise is comparable to the intrinsic shape noise, so the decrease in
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{run-bdj03mcal02-r50.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{run-bdj03mcal02-s2n.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{R-bdj03-bdj03stars.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{mc-select-bias-thresh.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{mc-select-bias-thresh-with-nocorr.eps}
