...
We expect sheared correlated noise to partly cancel in the estimator in
equation \ref{eq:estimator}, because the effect is averaged over both positive
and negative shears.  The correlated noise part will be different
for different galaxies, and should also cancel at some level in the
shear estimator.
...
remainder is amplified due to division by $2 \gamma$.  We thus expect a
contamination in the measured response
\begin{equation}
    R^{\prime}  =  R + R_{\eta}.
\end{equation}
...
of the response.  We generate images with the right noise level and then
measure the response of this noise to a shear.  We then subtract the mean noise
