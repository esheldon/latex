...
measure $R^{\mathrm{after}}$.  These response when adding noise before
\mcal\ is given by
\begin{align}
    R^{\mathrm{before}} &= R(n + \Delta n) + A (n + \Delta n)^2 \nonumber \\
       &\simeq R(n + \Delta n) + A n^2 + \frac{\partial (An^2)}{\partial n} \Delta n + ... \nonumber \\
       &\simeq R(n + \Delta n) + A n^2 + 2 A n \Delta n
\end{align}
where we have dropped terms of order $(\Delta n)^2$ and higher.
The response when adding noise after \mcal\ does not suffer any additional
bias due to correlated noise:
\begin{align}
    R^{\mathrm{after}} &= R(n + \Delta n) + A n^2.
\end{align}
...
We apply each correction scheme to a set of image simulations.  These
simulations make use of parametric models.  The galaxies are bulge+disk models
with both components having the same ellipticity and half light radius, drawn
from a log-normal distribution centered at 0.7 arcsec, with scatter of 0.13
arcsec.  The bulges are offset uniformly up to a scale length.  The
ellipticities were drawn from the simple model presented in \cite{ba14}, with
$\sigma=0.2$.  The PSF is an elliptical Gaussian with FWHM 0.9 arcsec.  The
pixel scale was set to 0.265 arcsec.  The S/N distribution matches real DES
data, with $S/N \gtrapprox 10$.  Galaxies were sheared with 300 different
...
\begin{figure}
    ...
    \caption{Trend of $\Delta R$ with $2 n \Delta n$, where $n$ is the original
    noise level and $\Delta n$ is the additional noise added.  The trend is
    linear  as predicted.}
