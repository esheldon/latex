...
robustness of \mcal\ to stellar contamination, we included
...
refer to the simulation with stars as \bdstar; the \bdsim\ simulation is
identical but with the stars removed.
...
the galaxies randomly rather than placing them in pairs to cancel shape bias.
Using random orientations avoids cancellation of systematics such as some
selection effects \citep{DESSVShear}.  Second, we used optical aberrations in
the PSF designed to match that seen in the Dark Energy Survey
data\footnote{Aaron Roodman, private communication}.  Similar to GREAT3, we
varied the aberrations as Gaussian around a fiducial value. These
root-mean-squared variations, in units of waves in the Noll convention are
given in table \ref{tab:aberr}.  We used a Kolmogorov model for the atmospheric
component, with mean FWHM = 3.40 pixels (0.9 arscec for 0.265 arcsec pixels).
For this configuration there are significant variations in the PSF ellipticity,
but relatively little net ellipticity across the entire ensemble of
realizations.  The code used to generate these simulations began as a fork of
the GREAT3 public code base, and is freely available
...
is about \cosmosrdist, somewhat larger than the
log-normal \galrdist\ for the parametric simulations.
...
\begin{table*}
    ...
    \caption{Description of the image simulations.  For the \bdsim\ simulations,
        ...
        bad pixels, drawn from DES data. For the \rgsim\
...
produce a stable fit; for all images our entire set of simulations we had not a
...
net PSF anisotropy in the $g_2$ component, we detected a remaining
...
on a single Gaussian fit; we advise using a better model.
...
\ref{sec:bdsim}.  In this simulation, we masked ``bad'' pixels and columns,
with masks drawn from real DES data.
