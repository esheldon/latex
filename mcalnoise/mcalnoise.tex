...
If we have estimates of the values of \mcalR\ for each galaxy, we
...
\end{equation}
In this work we instead correct for any PSF anisotropy by re-convolving by
a circular PSF.
...
associated with the particular estimator used therein.  For the estimator
...
\end{align}
We will assume the
integral over the product $P(\mest) S(\mest)$ is normalized to unity.

Following \S \ref{sec:mcal}, we wish to derive the response
of the estimator to a shear by taking the derivative about
zero shear:
\begin{align}
...
\end{align}
We can approximate the derivatives using finite differences in the shear.
Using the notation for measurements on sheared images, we get
\begin{align}
    \frac{\partial \langle \mest \rangle}{\partial \gamma}\bigg|_{\gamma=0}
    &\approx
    \int d\mest \left [ S \left( \frac{ \mest^+ P^+ - \mest^- P^- }{\Delta \gamma}\right) + \mest P \left( \frac{ S^+ - S^- }{\Delta \gamma}\right) \right] d\mest \nonumber \\
    &=\frac{\langle \mest^+ S \rangle - \langle \mest^- S \rangle}{\Delta \gamma} + 
\frac{\langle \mest S^+ \rangle - \langle \mest S^- \rangle}{\Delta \gamma} \nonumber \\
       &\equiv \langle R \rangle + \langle \mbox{\mcalRS} \rangle.
\end{align}
The first term is the response of the mean ellipticity to a shear, \mcalR, as
discussed in \S \ref{sec:mcal}, with selections based on the unsheared
object parameters.  The second term is the response of the mean ellipticity to
selections.  
...
with selection based on the unsheared parameters.  Similarly, the $\langle
\mest S^+ \rangle$ represents the mean of the unsheared ellipticities, with
selection based on the sheared parameters, and $\langle \mest S^- \rangle$
represents the mean of the unsheared ellipticities, with selection based on the
negatively sheared parameters.  In practice, we recommend storing all the
sheared and unsheared measurements and perform separate selections and
statistics on those values to compute any ensemble averages needed for a
particular inference (see e.g. \S \ref{sec:Rtwopt} where these quantities are
used differently).
...
\begin{align}
    \xi &= \langle e_\alpha e_\beta \rangle \nonumber \\
        &= \int P(e_\alpha e_\beta) e_\alpha e_\beta d e_\alpha d e_\beta
\end{align}
where the indices $\alpha, \beta$ indicated two different components of
the ellipticity. Note the probability distribution $P$ is joint the
the two ellipticity components. Including selection effects, this becomes
\begin{align}
        \xi &= \int P(e_\alpha e_\beta) S_\alpha S_\beta e_\alpha e_\beta d e_\alpha d e_\beta
\end{align}
where the selections are independent on each component.

Assuming ellipticities are not correlated in the absence of shear,
the leading surviving term in the taylor expansion is a second derivative
\begin{align}
    \xi \approx \frac{1}{2!} \left< \frac{\partial^2 \xi}{\partial \gamma_\alpha \partial \gamma_\beta} \gamma_\alpha \gamma_\beta \right>
\end{align}
As with the mean shear, this equation looks like the true correlation function has
been weighted by a response.  We can expand this response as
\begin{align} \label{eq:twoptres1}
    \frac{1}{2} \frac{\partial^2 \xi}{\partial \gamma_\alpha \partial \gamma_\beta} =
        \frac{1}{2} \int d e_\alpha  de_\beta & \biggl[ \frac{\partial S_\alpha}{\partial \gamma_\alpha} \frac{\partial S_\beta}{\partial \gamma_\beta} P_{\alpha \beta} e_\alpha e_\beta  \nonumber \\
        ...
        & + S_\alpha S_\beta \frac{\partial^2 P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha \gamma_\beta}  \biggr].
\end{align}
Note the second and third terms are equal under swapping of the indices
and thus can be combined.

The first term in equation \ref{eq:twoptres1} represents the effects of independent
selections in each component.  The last term represents the response of the
estimator to shearing.  The cross terms represent the response to shearing
in one component and selection in another.

We can compute these terms using the sheared measurements, and using selections
on sheared measurements. The first term we can calculate using finite differences,
by repeating the measurement when selecting on sheared parameters
\begin{align}
    \int d e_\alpha & d e_\beta  \frac{\partial S_\alpha}{\partial \gamma_\alpha} \frac{\partial S_\beta}{\partial \gamma_\beta}  P_{\alpha \beta} e_\alpha e_\beta \approx \nonumber \\
...
\end{align}
These are three separate measurements to be made:  The first and last are auto-correlations where selections
are placed on the positively and negatively sheared parameters, respectively.
The second is a cross correlation between one catalog selected using positively
sheared quantities, the other using negatively sheared quantities.

We can follow the same procedure for the cross term, expanding the deriviatives as
finite differences:
\begin{align}
    \int d e_\alpha  d e_\beta  2 S_\alpha \frac{\partial S_\beta}{\partial \gamma_\beta} & \frac{\partial P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha} \approx \nonumber  \\
...
\end{align}
This term is thus calculated from four cross-correlations, with one of
the catalogs selected on sheared or unsheared parameters.  {\color{blue} TODO It may be this
can be simplified in terms of the finite shape difference for each object
$R_\alpha = (e_\alpha^+ - e_\alpha^-)/\Delta \gamma$.
\begin{align}
    \int d e_\alpha  d e_\beta  2 S_\alpha \frac{\partial S_\beta}{\partial \gamma_\beta} & \frac{\partial P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha} \approx \nonumber  \\
    \frac{1}{\Delta \gamma} \Bigl( & \left< S_\alpha S_\beta^+ e_\beta R_\alpha \right> - \left< S_\alpha S_\beta^- e_\beta R_\alpha \right> \Bigr).
\end{align}
If so, then this term is calculated from two cross-correlations, with one of
the catalogs selected on sheared or unsheared parameters.
}

The last term in equation \ref{eq:twoptres1} is a second derivitive
\begin{align}
    \int d e_\alpha  d e_\beta S_\alpha S_\beta & \frac{\partial^2 P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha \gamma_\beta} \approx \nonumber \\
    \frac{1}{\Delta \gamma^2} \Bigl(  & \left< S_\alpha S_\beta e_\alpha^+ e_\beta^+ \right> - \left< S_\alpha S_\beta e_\alpha^+ e_\beta^- \right> \nonumber \\
                                    - & \left< S_\alpha S_\beta e_\alpha^- e_\beta^+ \right> + \left< S_\alpha S_\beta e_\alpha^- e_\beta^- \right> \Bigr)
\end{align}
Note the cross terms are equal.  This term can thus be measured via
two auto-correlations and one cross-correlation.  {\color{blue}
TODO Again, it may be we can simplify
this in terms of the individual $R$ values as
\begin{align}
    \int d e_\alpha  d e_\beta S_\alpha S_\beta \frac{\partial^2 P_{\alpha\beta} e_\alpha e_\beta}{\partial \gamma_\alpha \gamma_\beta} \approx 
    \langle S_\alpha S_\beta R_\alpha R_\beta \rangle.
\end{align}
}
...
and disk, with additional knots of star formation.  We used different
ellipticities for the bulge and disk components, but gave both bulge
and disk the same half light radius \hlr.
...
to pure knots, which resemble elliptical galaxies.

The entire composite bulge, disk, knots object was sheared with $g1=0.02$ in
reduced shear units.

We convolved the galaxy images with a PSF modeled as a Moffat profile
\citep{Moffat1969}, with $\beta=3.5$ and FWHM=0.9 arcsec, with 0.263 arcsec
pixels. The PSF ellipticity was set to $g_2 = 0.025$ in reduced shear units ($e_2 =
0.05$ in units of distortion). 

These convolved objects were rendered onto a pixel grid and constant Gaussian
noise was added, with the same noise level used for all images.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{run-bdj03mcal01-r50.eps}

    \caption{Distribution of half-light-radius \hlr\ in the parametric simulations,
        for both galaxies and PSF.  These \hlr\ were draw from fits
        to COSMOS data.  Also shown is the \hlr\ for objects that survived
		the initial $S/N > 5$ cut.}
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{run-bdj03mcal01-s2n.eps}
    ...
    \snr$ > 5$. the measured \snr\ is biased and noisy,
...
from 23.5 magnitude limited COSMOS data.
...
the overall mean FWHM $\sim$ 3.40
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{mcal-v14s01-s2n-and-r50.eps}
...
include it to directly address any additional complexities in real galaxies
that may not appear in our bulge+disk+knots simulation, and to test shear
...
We fit the images with a single Gaussian galaxy model using the \ngmix\
code\footnote{https://github.com/esheldon/ngmix}.  To perform the fit we use an
...
In order to correct for PSF anisotropy, we had two choices:  we could apply
corrections using the \mcal\ PSF response terms, or we could reconvolve using a
round PSF.  In what follows we discuss only the latter, but note we also have
recovered the shear accurately in these sims using the former.

To create a round, symmetrized version of the PSF, we added the PSF image to
itself, rotated by 90, 120, and 180 degrees.  We further dilate this
symmetrized PSF by a factor $1+2*e$, where e is the measured ellipticity, to
ensure that the final PSF is larger than the original in all directions.


All \mcal\ operations were performed using the \texttt{metacal} module from
\ngmix, which in turn uses GALSIM to perform most image manipulations.
We added a noise field designed to correct for correlated noise, as discussed
in \S \ref{sec:fixnoise}
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{mcal-v14s01-v14s02-compare-R.eps}
...
\begin{table*}
    ...
    \begin{tabular}{ |l|  c|c|c|  c|c|c|}
        ...
        \hline
        \bdsim & $-496$      & $-0.02 \pm 0.04$ & $ 15.16 \pm 0.04$ & $-0.15 \pm 0.19$  & $-0.05 \pm 0.10$ & $0.17 \pm 0.10$  \\
        \bdstar& $-542$      & $-0.03 \pm 0.04$ & $ 18.33 \pm 0.04$ & $-0.14 \pm 0.20$  & $-0.09 \pm 0.10$ & $0.15 \pm 0.10$  \\
        \bdmask& $-496$      & $+0.09 \pm 0.10$ & $ 15.07 \pm 0.10$ & $-0.60 \pm 0.46$  & $-0.17 \pm 0.23$ & $0.73 \pm 0.23$  \\
        \rgsim & $-436$      & $-0.23 \pm 0.16$ & $~~0.32 \pm 0.16$ & $+0.22 \pm 0.58$  & $+0.26 \pm 0.29$ & $0.22 \pm 0.29$ \\
    \end{tabular}
\end{table*}
...
We show the effect of selections in table \ref{tab:results_sel}.
...
These results are also show graphicall in figures \ref{fig:s2nthresh} and \ref{fig:s2nrange}.
...
shear analysis if the expected stellar fraction is low.  Attempting to remove
faint stars from a sample is a noisy procedure, likely to induce selection
effects. For example, stars that are included in the sample after a size cut
will have fluctuated to larger measured size due to noise, and will also likely
have fluctuated to higher response \mcalR, biasing the recovered shear.
