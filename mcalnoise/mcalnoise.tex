...
correction for correlated noise.  We tested this formalism in simulations using
real galaxy images, as well as very challenging parametric simulations that
included pre-selection effects and stellar contamination.  We applied a range
of cuts on the object \snr, inducing significant selection affects.  In all
cases we recovered the input shear to better than a part in a thousand.

The primary remaining issues to be address to apply \mcal\ to read imaging data
are the effects of overlapping objects and the limitations of current image
processing techniques.

The processing pipeline that determines the background, identifies unique
detections, and assigns light to objects will potentially produce biases in the
shear recovery.  With \mcal\ these issues can be addressed in principle. Rather
than work on sheared postage stamps, the entire image can be sheared, and the
full processing framework rerun to estimate the response.  The primary
difficulties are the computational expense, and the difficulty in dealing with
image artifacts that cannot be deconvolved.
...
response for these "objects", but in order to interpret the signal the
redshifts of each object must be known.  If the galaxies are so blended that
they are considered one object by the pipeline, it is unlikely an accurate
redshift can be determined.  This is a problem shared by any shear measurement
pipeline, and will require some kind of external calibration.  These issues may
be best addressed by inserting artificial galaxy images into the data using the
recently introduced \balrog\ technique \citep{Balrog2016}.
