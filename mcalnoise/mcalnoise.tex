...
\citep{Bernstein2010}.  The second is the bias in the non-linear fit caused
by noise, which is significant for low signal-to-noise (\snr) images, and is
difficult to predict \citep{HirataAlign04,Refreg12,Melchior12}.  This ``noise
bias'' is worsened if large number of parameters are used in an attempt
to accurately represent the galaxy.
...
\citep[e.g.][]{Zuntz13,Miller13,KidsShear2016,Refregier13,Jee16}.  These
...
can partly be alleviated, along with noise bias, within a sufficiently flexible
statistical framework \citep{SchneiderProbshear2015}, without simulations for
calibration, although control of model bias has not yet been demonstrated.
...
images from a high \snr\ deep data is included.  Un-normalized moments in
...
corrected for naturally as part of the method, without use of external
...
A similar idea to \mcal\ was introduced by \cite{Kaiser2000}, although
the full equivalent of \mcal\ was not implemented therein.
...
Suppose we have a biased, noisy shear observable \vest, which may be some estimate of
...
right hand side.  As mentioned, the estimator \vest\ can be noisy, but in
principle when averaging over large ensembles this noise does not cause any
bias because \mcalR\ is very well determined (see \S \ref{sec:differences}).
However, if the \mcal\ process alters the noise in a coherent way, an
additional correction is required (see \S \ref{sec:fixnoise}).
...
to the choice of applied shear.  We tested values
in the range of 0.001 to 0.05 and did not see significant
...
diagonal,  in which case the correction in equation \ref{eq:rcorr} reduces to
element-wise division.  One might expect the response to be proportional to the
identity matrix if there is no preferred direction in the measurement process.
In the simulation tests we will present in \S \ref{sec:sims}, we found the
diagonal elements can differ by as much as a few parts in a thousand due to the
strong PSF ellipticity $\vest = (0, 0.025)$.  Additionally, some measurements,
such as tangential shear or pairwise two-point functions, require projecting
the ellipticities into different coordinate systems. The measurement may
require projection of the response matrix as well.
...
In \cite{HuffMcal} the simple averaging equation \ref{eq:rcorr} did not work
because the shape estimators used  were quite noisy.  Instead a sophisticated
...
measured, even for very low \snr\ galaxies, and the average \mcalRmean\ is very well determined. We find that
using simple averages in equation \ref{eq:rcorr} are adequate to infer the
correct shear. 
...
effects. First we introduce some notation:  In what follows, we will, without
loss of generality, let the shear estimator \vest\ be the two component
ellipticity $\vest = (\est_1,\est_2)$\footnote{With \mcal\ the estimator need
not be a measure of ellipticity}.  We write the ellipticity measured from a
sheared image as $\vest^+$ and $\vest^-$, for measurements on positive and
...
The continuous approximation should be valid in the limit of a large ensemble.
We will present all calculations as ensemble averages, which can be realized
as sums over discrete measurements in real data.
...
$S(\vest)$ in \ref{eq:Rmean} will be infinite.  It is thus important to avoid
