...
non-linear model fitting caused by noise, which is significant for images with low
...
methods as a result of the non-linear fitting involved with the measurement.
...
same ellipticity assigned to the disk; but note this does not result in a
...
objects otherwise oriented.  Thus a pre-selection will tend to alter the
...
pre-selection, we did not perform the \mcal\ process on all images.  We applied
a pre-selection on objects with {\em measured} \snr$ > 5$, and these objects
...
this threshold in order to minimize the bias due to removal of these galaxies.
...
by the \psfex\ package \citep{BertinPSFEx2011}.  Note, rather than
use a round reconvolution function, one may instead use
the response of the estimator to a psf shear to address any uncorrected
affects of PSF anisotropy \citep{HuffMcal}.
...
pre-selection to the \bdksim\ simulation at measured \snr$ > 5$, which imposes
