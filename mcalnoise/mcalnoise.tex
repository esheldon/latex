...
zero shear:
\begin{align}
    \mbox{\mcalRmean}  = \frac{\partial \langle \mest \rangle}{\partial \gamma}\bigg|_{\gamma=0} 
...
\frac{\langle \mest S^+ \rangle - \langle \mest S^- \rangle}{\Delta \gamma} \nonumber \\
       &\equiv \langle \mbox{\mcalRg} \rangle + \langle \mbox{\mcalRS} \rangle \equiv \mbox{\mcalRmean}.
\end{align}
The first term \mcalRgmean\ is the response of the mean ellipticity to a shear,
as discussed in \S \ref{sec:mcal}, with selections based on the unsheared
object parameters.  The second term \mcalRSmean\ is the response of the mean
ellipticity to selections.  
...
\end{align}
where the selections are independent on each object.  Note the probability
distribution \probe\ is joint in the two ellipticities because the shear at
each galaxy $\alpha$ is correlated with that at galaxy $\beta$. We assume this
distribution to be separable in the absence of shear, in other words galaxy
shapes are not intrinsically correlated.  If we further assume the mean shear
is zero over a large area of sky, the leading surviving term in the Taylor
expansion is a second derivative
\begin{align} \label{eq:basictwopt}
    \xi \approx \left< \frac{\partial^2 \xi}{\partial \gamma_\alpha \partial \gamma_\beta}\bigg|_{\gamma=0} \gamma_\alpha \gamma_\beta  \right>
\end{align}

As with the mean shear estimator, equation \ref{eq:basictwopt}
looks like the true correlation function has
been weighted by a response, where the response is given by
\begin{align}
...
\end{align}
Although this response is evaluated at zero shear, the argument does not,
generally, factor into independent parts for $\alpha$ and $\beta$.
In particular, the derivatives of the probability distribution
\probe\ do not necessarily exactly factor.  However, we expect the
response to be of order unity, while any coupling between $e_\alpha$ and $e_\beta$ 
is of order the product $\gamma_\alpha \gamma_\beta \sim 10^{-4}$.
Thus, for purposes of the calculation of the response, we can
factor this equation to good approximation:
\begin{align}
    \int d e_\alpha  de_\beta  & \frac{\partial^2 S_\alpha S_\beta \mbox{\probe} e_\alpha e_\beta}{\partial \gamma_\alpha \partial \gamma_\beta}\bigg|_{\gamma=0} 
      \nonumber \\
...
\end{align}
These two responses are statistically identical, and are each equal to the
