...
method involves introducing a counterfactual shear to a galaxy images, and
...
%    \centering
    ...
    \includegraphics[width=\textwidth]{mcal-v14s01-s2n-and-r50-with-nsim.eps}
...
as red diamonds. The bias after correction for selection effects is represented
as blue circles.  The filled grey region represents the target accuracy. } 
...
only small fraction of images should be affected by missing data such as bad
...
We tested the bias from this non-linearity using simulations.  Rather than
process the large image simulations presented in \S \ref{sec:sims} using
different shear values, we instead tested the \mcal\ formalism using a
simplified and fast ``toy model'' simulation, inspired by that used in
\citep{ba14}.  We drew ellipticities from the distribution given in equation
...
making the noise effectively non-Gaussian.
...
\begin{figure}
	...
	as expected, with $\alpha=1$ and zero offset. For the
	image simulation XXX.  The gray
...
other technique of which we are aware that has demonstrated shear recovery with
similar accuracy, without reliance on calibration from simulations, and which
...
challenging simulations and has proved more accurate.  \Mcal\ also has the
...
bright stars that cannot be deconvolved.
...
redshifts of the objects must be known, at least statistically.  If a
significant number of galaxies are so blended that they are considered one
object by the pipeline, it is unlikely an accurate redshift distribution can be
determined.  Addressing this problem may require some kind of external
calibration.
...
estimation \citep[e.g using the \balrog\ technique of][]{Balrog2016}.  This method has the advantage of using the
...
the variation of the optical aberrations measured in DES data.  
