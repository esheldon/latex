...
stars as \bdstar; the \bdsim\ simulation is identical but with the stars
removed.
...
the skewness dramatically from -1.26 to -0.30.
...
errors for the all simulations. For the parameteric simulations, which had a
...
the \bdsim\ simulation.  The results are shown in table \ref{tab:results}.  We
did not detect any additional bias due to including stars.
...
and galaxies.  Indeed we see that \mcalR\ is noisy but consistent with
zero for the stars.
...
detect any significant additional bias.  However, including \nsimNstarperc\
stars did increase the bias by about 5\%.
...
included in the sample after a size cut will have fluctuated to larger measured
size due to noise, and will also likely have fluctuated to higher response
\mcalR, biasing the recovered shear.
...
could also be down-weighted using an appropriate weighting scheme, but it
should be noted that weighting will generally cause a bias: weighting
is just a smooth selection.
...
\ref{sec:bdsim}.  In these simulations, we masked bas pixels and bad columns,
...
correlated noise bias at the 1-2\% level rather than the 12\% we saw in our \rgsim\
