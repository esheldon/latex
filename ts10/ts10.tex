...
is found to be good to the $\sim$1\% level.
...
target selection.    These are all described in the above references in this
section, but we will give a few more details here.
...
This initial selection is a pure morphology based algorithm.  We first define
the concentration $c$ as the difference between the magnitude as determined
from the best fitting PSF model (\psfmag) and the magnitude determined
from the \modelmag:
\begin{equation}
...
concentration for galaxies should be larger.  We define a simple cut $c >
0.145$ for galaxies.

We tried to implement an alternative star galaxy separation scheme for the
galaxy target selection, relying on the fact that the galaxy targets
should be better measured in \imag\ than other bands, but did not
...
practice stars are by far the largest contaminant in the quasar target sample.
