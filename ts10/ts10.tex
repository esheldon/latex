...
\tablecaption{Definition of \boss\ Survey Footprint\label{tbl:surveybounds}}
\tablewidth{0pt}
\tablehead{
    \colhead{Region} &
    \colhead{$\eta_c$ min} &
...
are defined in corrected survey coordinates.  Positions inside
these rectangles are included in the survey. See also figure 
\ref{fig:footprint}}
\end{deluxetable}
...
\begin{figure}[t] \centering
 ...
 amplifier behind one of the \umag-band filters was not working.  We use this
 mask during quasar target selection.} \label{fig:badu}

\end{figure}


\begin{deluxetable}{lrrrrr}
\tablecaption{Masked Areas\label{tbl:masked}}
\tablewidth{0pt}
\tablehead{
    \colhead{Region} &
    \colhead{$\eta_c$ min} &
...
\enddata
\tablecomments{Regions excluded from the quasar target
selection due to malfunctioning amplifier behind a
\umag-band filter.  See also \ref{fig:badu}}
\end{deluxetable}
