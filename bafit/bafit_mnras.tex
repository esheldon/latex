...
that of the PSF, with log-normal scatter.  I generated fluxes with log-normal
scatter and a discrete set of mean fluxes tuned to produce various optimal,
matched signal-to-noise ratios in the range $10-1000$.  I applied a constant
...
fractional error $\Delta g/g < 2 \times 10^{-3}$ in all cases, confirming
that, in these controlled conditions, the method is sufficiently unbiased
for planned surveys.
...
to derive point estimators for which the average is unbiased, even in the
presence of large noise, but no such estimator currently exists.
...
%signal-to-noise ratio (\sn), and is thus difficult to interpret.  Even if only
%high \sn\ galaxies could be used, 
...
bias.  Going forward, such simulation efforts will be important, but for this
preliminary work I sought more controlled conditions.
...
\end{equation}
The full elliptical model has six parameters: two centroid parameters, two
ellipticity parameters, a size parameter and an amplitude, or flux parameter.  I
used two distinct values of the index: $n=1$ (exponential disks), and $n=4$
(\devauc\ profiles).  
...
the mean FWHM of the galaxies to be approximately 1.2 times that of the PSF, I
was attempting to mimic the sort of noisy size cut that can occur when
...
convolutions; for Gaussian convolutions, the covariance matrices add.  This is
a fast and simple alternative to convolutions using Fourier transforms.
...
found that increasing the number of steps generally resulted in less bias,
but I found diminishing returns beyond 200 for the simple models I was
fitting.
...
resources used.  Graphics processing units may prove particularly useful; in
a preliminary study I found an increase in speed of order 100 over traditional
processors.
...
zero shear in Equations \ref{eq:pexpand} and \ref{eq:pqrdef}.  I performed
limited tests expanding about zero shear and shearing the images by only
$g=0.01$, and got comparable results. 
...
by \citet{HutererSystematics06}, assuming fiducial sky coverage and less than
20\% degradation in the accuracy of the recovered dark energy equation of state
parameter, relative to shot noise errors, due to bias in shear estimation.  The
cut at 20\% is somewhat arbitrary, and was chosen to coincide very roughly with
the stated requirements for DES. The requirements are $\sim$\desreq\ for
...
necessarily introduce additional selection effects.
...
\matR\ parameters.  I leave tests with more complex models to future work.
