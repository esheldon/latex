...
``shear'' derived from an ensemble of galaxy images, rather than a single
galaxy shape, and hence holds promise to accurately recover the shear when
using galaxies at very low signal-to-noise ratio.  Both the simulated and fit
models were drawn from the \sersic\ family of approximate galaxy profiles.  The
shear can be accurately recovered for a range of model types and galaxy sizes,
as long as the galaxy {\it size} is precisely determined, as parametrized by
the signal-to-noise ratio \Tsn.   The more standard signal-to-noise ratio of
the flux is not a good indicator of shear accuracy.  For \Tsn$\gtrsim 8-10$ the
shear can be recovered to an accuracy better than $\sim 10^{-3}$, sufficient
for current generation lensing experiments.  \end{abstract}
