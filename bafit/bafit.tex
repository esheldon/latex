...
the potential for accurate shear recovery from low signal-to-noise ratio (\sn)
galaxy images.  In that work the authors confirmed the method is unbiased by
drawing shapes from a known distribution, shearing them, adding noise, and
recovering the average input shear using the shape distribution as prior
information.  Here I present a full implementation for fitting galaxy images,
including point spread function (PSF) and pixelization effects.  I tested the
implementation using simulated galaxy images modeled as \sersic\ profiles with
index $n=1/2, 1, \textrm{and} ~ 2$, convolved with a PSF and a flat pixel
response function.  I used a simple Gaussian model for the PSF to avoid
potential PSF-fitting errors. I simulated galaxies with a discrete set of size
ratios $\sigma_{gal}/\sigma_{PSF} = 1, 1.4, 2$, a discrete set of average $\sn\
\in [10,1000]$, and applied a constant shear to all images. I fit the simulated
images to a model with the true \sersic\ index to avoid modeling biases.   In
all cases I recovered the input shear to better than 2 parts in a thousand,
finding the largest bias for the smallest galaxies. This accuracy is sufficient
for current and next-generation surveys.   
