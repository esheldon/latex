...
galaxy will not generally be Gaussian except in the limit of exceedingly high
...
estimation. Some kind of averaging most be performed to extract a shear from
...
Under these assumptions, \cite{ba14} derived a second-order estimator for the
shear, consistent with the assumption of a Gaussian posterior.  This form is a
Taylor expansion of the logarithm of the posterior for the shear, with shear as
...
are third-order tweaks to the second-order perturbations, but I will leave
implementations of those ideas to future work.
...
For this work I implemented the \citet{ba14} to analyze noisy galaxy images
smeared by a point spread function and a pixel response function.
...
surface requires a large number of model evaluations.  As an optimization, I
approximated galaxy models as sums of Gaussians according the to the fits in
\citet{HoggGMix12}.  I also fit the PSF using Gaussians.  By using Gaussians
for both galaxy and PSF models I could perform fast analytic convolutions of
the galaxy model with the PSF.  This is a faster and simpler alternative to
convolutions using Fourier transforms. The use of the Gaussian approximation
introduces negligible bias in the recovered shear for the \sersic\ models fit
in this work.
...
that with these settings accurate results were obtained for the full range of
parameters used in the simulations, so I did not explore this further.
Certainly optimization if the likelihood exploration is worthy of study.

Note I did not apply the prior on the galaxy shapes during exploration of the
...
for the other parameters to restrict the space.
