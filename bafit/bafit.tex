...
smallest galaxies tested, which have second moment size equal to the PSF.  If
this bias were related to noise bias I would expect the error to be worse for
low \sn\ images, but in fact the bias is similar at intermediate and high \sn.
This may be a sign that the implementation itself has problems, perhaps the
exploration of the likelihood using MCMC.  More study will be required to
track this issue down.

In \cite{ba14} it was noted that small and noisy galaxies will get little
weight in the average shear over an ensemble of galaxy images with a wide
variety of \sn.  This is intrinsice to the estimator, and does not require
any additional weighting.  A more realistic simulation, with galaxy images
drawn from a flux-limited sample, will test this reasoning.

In Figure \ref{fig:nonoise} I showed the multiplicative error coming from the
...
recover larger shears to sufficient accuracy.  Also for estimation of the shear
over relatively small patches, in which the shear is constant, an iterative
appoach is feasible where initially the expansion about zero shear is used, and
on a second iteration the mean shear from the first iteration is expanded
about.
