...
In the figures above I showed grey bands representing the approximate
...
Requirements for current surveys are met in all cases.

For planned surveys my implementation does not meet requirements for the
smallest galaxies tested, which have second moment size equal to that of the
PSF.  If this bias were related to noise bias I would expect the error to be
worse for low \sn\ images, but in fact the bias is similar at intermediate and
high \sn.  This may be a sign that the implementation itself is flawed; perhaps
the exploration of the likelihood using MCMC needs further tuning.  The
simulations used in this study took millions of processing hours, so repeating
the experiments to the same precision for new MCMC parameters is a significant
...
weight.  It may be worthwile to make cuts on size, but care must be taken not
to introduce selection biases. A more realistic simulation, with galaxy images
drawn from a flux-limited sample, can test this reasoning, but is beyond the
scope of this work.

In Figure \ref{fig:nonoise} I showed the multiplicative bias due to the
breakdown of the second-order taylor expansion at higher shear.   This level of
error is prohibitive even for current surveys at shear about 0.05.  An
