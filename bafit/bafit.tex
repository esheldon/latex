...
scatter, and similarly a discrete set of mean $\sn\ \in [10,1000]$.  I applied
a constant shear to all images. I fit the simulated images to a model with the
true \sersic\ index to avoid modeling biases.   I recovered the input shear
with sufficient accuracy for planned surveys except for galaxies with $n=2$,
size equal to the PSF size, and \sn\ $\lesssim 15$.  Most such galaxies will be
removed during star-galaxy separation, and the remaining will get little weight
in the mean shear, but further study is needed to access the impact on a real
survey.  This demonstration can be considered a proof-of-principle, and
