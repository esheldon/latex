...
%estimated \Nofz } \textcolor{red}{I think the area I marked in blue is too much
%detail for the abstract}.  
We expect the primary source of error is sample
variance: the training sets are drawn from relatively small volumes of space.
Using simulations we calculated the uncertainty in \Nofz\ at a given redshift
is $\sim$ \localbias.  The uncertainty on calculations using \Nofz\ or \pofz\
...
samples have different properties.  However, this does require that areas of
...
Finally, the light blue dot-dashed line shows the results when 3 neighbors are
used to estimate the \pofz, not including the galaxy itself.  This helps
ameliorate the problem we saw with abnormally broad $P(z)$ when using such a
small training sample and 100 neighbors.  The mean $P(z)$ for the 3 neighbors
case is 0.078, to be compared to the mean width of the DR8 $P(z)$s of 0.098. ;
now, the calibration bias is quite close to the ideal case with
$N(z)_\mathrm{wei}$, and the weighting is also as close to optimal as possible
for this sample {\color{red} how do you know that?}.

{\color{red} need to add discussion of n=7}.
...
\begin{figure} [h]\centering
    \includegraphics[scale=0.5]{figures/pz.n7.ps}
    ...
    to $1$  as possible.  {\color{red} need to label all curves in 
    the figure} \label{fig:simplebias}}
