...
growth of structure using photometric LRGs \citep{cro11}. %\citep[see e.g.][]{}and astrophysical analysis.
...
In figure \ref{fig:rand6pofz} we show 6 randomly chosen $P(z)$.  Each panel
contains a random $P(z)$ drawn from a particular magnitude range in extinction
corrected $r$-band \cmodelmag; these ranges are given in the figure caption.
This figure captures the general trend that the $P(z)$ are broader at
fainter magnitudes.

\begin{figure*} [t]\centering
    \includegraphics{figures/11-6pofz.eps}
    \caption{Randomly chosen $P(z)$.  In each panel, a random object was
    chosen from a particular magnitude range.  Columwise from top the
...
individual $p(z)$'s are strongly correlated with the width of the $p(z)$ \citep{CunhaPhotoz09}.
