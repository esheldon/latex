...
%Our training sample is not always matched to the best run in the Stripe82 samples, so that 
...
%If we include seeing as a variable in the weights estimation, the weighted seeing distribution of our training sample
%closely matches that of the photometric sample.
%This comes at the price of degrading the matching between the weighted and photometric r-mag distribution.
%In the end, the mean of $N(z_{wei}$ shifts by 
...
\begin{figure}[t]\centering
    ...
    \caption{\color{red}{Simulated redshift distribution of an $r<21.8$ sample.
The error bars are the $1-\sigma$ simulated estimates of the variability of the 
uncertainty in the redshift distribution due to sample variance in the catalogs 
