...
derive redshift probability distributions \pofz\ for individual galaxies.
\textcolor{blue}{  The \pofz\ summed over all galaxies is similar but not identical to the redshift distribution of the overall sample. We applied a correction to the individual
\pofz\ so that the summed \pofz\ equals the estimated \Nofz }
\textcolor{red}{I think the area I marked in blue is too much detail for the abstract}.  We expect the
...
as neural nets.  Forcing the \textcolor{red}{distributions of observables of } the two samples to be
...
have different properties.  However, this does require that areas of \sout{parameter} \textcolor{red}{observable}
...
\textcolor{red}{In what follows, we identify potential sources of systematics and detail our tests to constrain them:}
...
\textcolor{red}{For individual $P(z)$, the main source of uncertainty is shot-noise, because only
100 galaxies were used to estimate each $P(z)$.
The choice of fixing the number of neighbors keeps the shot-noise equal for all galaxies,
but can also yield biases or an artificial broadening of the $P(z)$ if the training set
is too sparse near the galaxy of interest.
However, we do not find that the volume spanned by the 100 nearest neighbors to be a good 
indicator of the $P(z)$ quality because other properties of the redshift-observable 
hypersurface affect the local density of galaxies and redshift correlations to the observables.
A potentially more interesting indicator of bias in individual $P(z)$ is the spatial distribution
of the training set nearest neighbors relative to the galaxy for which a $P(z)$ is needed.
We leave these explorations for a future work.
}
...
%please correct this description if wrong}.  
In this case, the individual $P(z)$
are on average $40\%$ broader than the DR8 $P(z)$ %{\color{red}quantify?}
because of the use of 100 neighbors to construct the $P(z)$ when the training
sample itself is only $7$ times as large.  
To compensate for the bias introduced by the small size of the training sample, 
we have imposed a multiplicative correction factor to the $P(z)$ such that $\sum P(z) =
...
removed using a calibration sample); but the \sout{weighting} \textcolor{red}{variance ratio} 
is still far closer to optimal than when we did not use weighting information, and even slightly
...
neighbors.
The mean $P(z)$ for the 3 neighbors case is 0.078, to be compared to the mean width of the 
DR8 $P(z)$s of 0.098.
%{\color{red} quantify new width?}
; now, the calibration bias is quite close to the ideal case with $N(z)_\mathrm{wei}$, and 
the weighting is also as close to optimal as possible for this sample.
