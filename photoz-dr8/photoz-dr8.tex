...
local color-magnitude space around each object.  Using these individual \pofz\
can reduce the scatter in measurements that depend on the redshift distribution
of individual objects.  The spectroscopic training sample is substantially
larger than that used for the DR7 release; of particular note is addition of
the PRIMUS catalog, which is the most important training set used in this
analysis.  Nevertheless, we expect the primary source of error is sample
...
$\sim$ \localbias.  The uncertainty on calculations using \nofz\ or \pofz\
depends on how they are used; we discuss the case of weak lensing measurements.
The \pofz\ catalog is publicly available from the SDSS website.  
