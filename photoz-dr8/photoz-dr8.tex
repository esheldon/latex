...
We derive redshift probability distributions for galaxies in the SDSS DR8
imaging data.  We use the ``weighting'' algorithm presented in
...
particular importance is the inclusion of the PRIMUS catalog. With this
technique, weights are derived for a set of training galaxies with known
redshifts such that their density distribution in color-magnitude space is
proportional to that of the photometry-only sample, producing a nearly ``fair
sample'' in that space.  The \Nofz\ of the photometric sample is then estimated
...
depends on the details; we discuss the case of weak lensing measurements.  The
