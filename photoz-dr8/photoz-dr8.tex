...
the overall sample. We apply a correction to the \pofz\ so that the summed
\pofz\ equals the estimated \Nofz.  The bias of \Nofz\ is relatively small in
an aboslute sense ($<$ \absbias), but it's effect depends on the type of
analysis performed.  As proof of principle, we use a simplified simulation
where only DEEP2-EGS is used as the training set, and estimate the expected
bias for a weak lensing study.  If the training set weights are derived
correctly, we would expect $<$ \worstlensbias\ in such a study for lens
redshifts $<$ 0.3.  The results would improve with the additional training sets
we have used, and proper error based weighting of the galaxies.  The catalogs
are available for download through the SDSS website.  
...
The resulting lensing calibration bias is shown in Fig. \ref{fig:simplebias}.

 - Calculate mean \pofz width for this sample?

\begin{figure} [h]\centering
    \includegraphics[scale=0.5]{figures/pz.egs.c3.paper.ps}
    \caption{No description yet. X label is a bit hard to read.  I
    think it would be clearer to limit the y range to $\pm 0.1$
    and forget about zebra.
...
\enddata
\tablecomments{Reconstructed redshift distribution \Nofz\
for SDSS galaxies with \rmag\ $ < $ \rmax.  The first two
columns specify the redshift range of the bin, the third is the
reconstructed \Nofz, and the fourth is the
sample variance errors derived from simulations.  These
sample variance errors should be thought of as a rough
estimate.  A more perfect match would require a simulation
more specifically tuned to the SDSS data.}
\end{deluxetable}
