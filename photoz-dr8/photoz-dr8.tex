...
{\it Le Phare} \citep{...}, {\it ZEBRA} \citep{}, {\it BPz} \citep{}, {\it
ArborZ} \citep{...}, and our own nameless method \citep{CunhaPhotoz09}.
...
\citep{car11} {\color{red} what about Padmanabhan?} and for the measurement of
...
set galaxies that fall within that bin.  {\color{red} What do you think of my
version of the equation?  I think it is a better parallel with the p(z)
equation} Ref.~\cite{LimaPhotoz08,CunhaPhotoz09} show that this indeed provides
...
to calculate the weights.  {\color{blue}For example, if the photometric sample
has some sort of size dependent cut, the same cut should be applied to the
training sample or size should be one of the observables used to measure
weights.}
...
\noindent This is the same as equation \ref{eqn:Nzest} but limited to the
nearest neighbors to the galaxy of interest. We estimate \pofz\ in {\color{red}
\sout{20}} 35 redshift bins between $z=0$ and {\color{red}\sout{$1.35$}} 1.1.
...
As detailed in \cite{CunhaPhotoz09}, the weights and \pofz\ are susceptible to
...
\begin{figure}[t]\centering
    \includegraphics[{angle = -90,scale=0.35}]{figures/nz.sim.error.ps}
...
\begin{figure*} [t]\centering
    \includegraphics{figures/11-6pofz.eps}
...
individual \pofz s are strongly correlated with the width of the \pofz\ \citep{CunhaPhotoz09}.
Broader \pofz\ reflects larger degeneracy in observable space, and require more training-set
...
the \pofz.  The contours indicate factor of 2 changes in density.  
...
\begin{figure*}[t]\centering
    \includegraphics{figures/pzwidth25bins.eps}
...
in this test case, even though the \pofz s are quite broad (because the training set is so small).
...
\end{equation}
discussed in Cunha et al 2011, in prep. to the \pofz s.
