% SYMBOLS.TEX -- AASTeX and LaTeX symbol tables.
%
% I have removed these from the main user guide document in order
% to facilitate the presentation of this material in a number of
% forms: hypertext, PDF crib sheets, etc.
%
% Note that by intention this document has no title, no author,
% no date, no sections, etc.

\documentclass[preprint2]{aastex}

\usepackage{aastexug}	% User guide style customizations
\pagestyle{symbolapp}	% Symbol table appendix page style

\markright{\aastex, AMS math, and \LaTeX\ symbols}
\symbolappfolioprefix{A}
\hoffset=-0.5in
...
$\gtrsim$ & \verb"\gtrsim", \verb"\ga" \\[1ex]
\micron & \verb"\micron" &
\sbond & \verb"\sbond" \\
\dbond & \verb"\dbond" &
\tbond & \verb"\tbond" \\
...
$\mu$      & \verb"\mu"      & $\omega$   & \verb"\omega"   \\[3ex]
...
\\[2ex]
%\multicolumn{4}{c}{Negated arrows} \\[1ex]
$\nleftarrow$ & \verb"\nleftarrow" &
$\nrightarrow$ & \verb"\nrightarrow" \\
$\nLeftarrow$ & \verb"\nLeftarrow" &
$\nRightarrow$ & \verb"\nRightarrow" \\
...
%\multicolumn{1}{l}{$\lesssim$} & \multicolumn{1}{l}{\tt lesssim} &
%\multicolumn{1}{l}{$\gtrsim$} & \multicolumn{1}{l}{\tt gtrsim} \\
...
$\updownarrow$ & \verb"\updownarrow" &
$\Updownarrow$ & \verb"\Updownarrow" \\[1ex]
